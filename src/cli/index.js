...
const getAppPath = userPath => {
  ...
  return inquirer.prompt(askAppPath).then(r => r.appPath);
};

getAppPath(appPath).then(applicationPath => {
  const optionsFromArguments = getOptionsFromArguments(options.rawArgs);
  const appName = optionsFromArguments.name || path.basename(applicationPath);
  ...
  async function run() {
    console.log();
    console.log(
      `Creating a new InstantSearch app in ${chalk.green(applicationPath)}.`
    );
    console.log();
    ...
    const app = createInstantSearchApp(applicationPath, config, tasks);
